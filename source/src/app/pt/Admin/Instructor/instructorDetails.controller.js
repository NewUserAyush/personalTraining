(function() {
    ...
    /* @ngInject */
    function fnCtrl($scope, $filter, $location, $state, $mdMedia, $mdBottomSheet, $stateParams, $mdDialog, $mdToast) {
        var vm = this;
        vm.detail = "details";
        vm.id=   $stateParams.id;

        vm.items = [{id:11,name:'jay',age:31,style:'dance',doj:"31/12/2015"},{id:12,name:'ayush',age:33,style:'math',doj:"31/12/2015"},{id:13,name:'vijay',age:25,style:'computer',doj:"31/12/2015"}]

        vm.item =   vm.items.filter(function(data){
            if(data.id === parseInt(vm.id))
            {
                return data;
            };
        })
        ...
        function openMail(item) {
            $state.go(vm.baseState.name + '.item', {
               itemID: item.id
            });
            item.unread = false;
            vm.selectedMail = item.id;
        }
